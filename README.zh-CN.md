...
> **选哪个项目？**
> - **本项目**（`feishu-kirocli-bot`）：仅支持飞书，轻量简单。
> - [`kirocli-bot-gateway`](https://github.com/terrificdm/kirocli-bot-gateway)：多平台支持（飞书 + Discord + 更多）。如果需要多平台或未来可能扩展，用这个。
...
- 群聊 @机器人 触发，私聊直接处理
- **📁 工作空间模式** - `per_chat`（用户独立）或 `fixed`（共享项目目录）
...
- **🔧 MCP 与 Skills 支持** - 全局或工作空间级配置（`.kiro/settings/mcp.json`、`.kiro/skills/`）
- 展示工具调用过程 + 最终回复
- 并发控制（同一会话同时只处理一个请求）
- kiro-cli 进程异常自动重启
...
- **群聊**：@机器人 + 你的问题
- **私聊**：直接发送消息
- **图片**：发送图片让 Kiro 分析（截图、架构图等）——支持 JPEG、PNG、GIF、WebP，自动格式检测，跨轮次持久化
- **取消**：发送「cancel」中断当前操作
